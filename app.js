...
// ---------- СОСТОЯНИЕ ----------
const state = {
  authUser: null,         // объект auth.users
  appUser: null,          // строка public.app_users
  group: null,            // строка public.groups
  sessionToken: null,     // active_session_token из try_start_session
  heartbeatTimer: null,
  currentPage: null
};
...
async function signInWithGoogle() {
  ...
  if (error) toast('Ошибка входа: ' + error.message, 'error');
}
...
// best-effort завершение сессии при закрытии вкладки
...
async function fetchAppUser() {
  ...
  if (!data && !error) {
    // handle_new_user триггер может ещё выполняться — ждём и пробуем снова
...
async function fetchGroup() {
  ...
  const { data } = await sb.from('groups').select('*').eq('id', state.appUser.group_id).maybeSingle();
...
async function loadPage(key) {
  ...
  try {
    ...
    // Файл ещё не создан — показываем заглушку
    container.innerHTML = `
...
async function boot() {
  ...
  await fetchAppUser();
  if (!state.appUser) {
    toast('Профиль пользователя не найден. Попробуйте позже или обратитесь к администратору.', 'error');
    await signOut({ silent: true });
    return;
  }
...
function wireEvents() {
  ...
  $('btn-signout-main')?.addEventListener('click', () => signOut());
