...
import { formatDate, formatDateTime, formatMoney, formatNumber, formatQuantity } from './format.js';
...
// ---------- REALTIME: уведомления о новых membership ----------
let membershipsChannel = null;

function subscribeToMemberships() {
  if (membershipsChannel) return;
  if (!state.authUser) return;

  membershipsChannel = sb.channel('memberships:' + state.authUser.id)
    .on('postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'group_memberships', filter: `user_id=eq.${state.authUser.id}` },
        async (payload) => {
          log('realtime: new membership', payload.new);
          const row = payload.new;
          // Обновляем локальный список групп
          await fetchMyGroups();
          renderGroupSwitcher();
          // Находим название группы
          const group = state.myGroups.find(g => g.group_id === row.group_id);
          const name = group?.group_name || 'новую группу';
          toast(`Вас добавили в группу «${name}»`, 'success');
        })
    .on('postgres_changes',
        { event: 'DELETE', schema: 'public', table: 'group_memberships', filter: `user_id=eq.${state.authUser.id}` },
        async (payload) => {
          log('realtime: membership deleted', payload.old);
          const removedGroupId = payload.old?.group_id;
          await fetchMyGroups();

          // Если удалили текущую группу — надо переключиться или выкинуть
          if (state.currentGroup && state.currentGroup.group_id === removedGroupId) {
            toast('Вас исключили из текущей группы', 'warning');
            if (state.myGroups.length > 0) {
              await switchToGroup(state.myGroups[0].group_id);
            } else {
              showScreen('auth'); showAuthView('no-groups');
            }
          } else {
            renderGroupSwitcher();
          }
    ...
    .on('postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'group_memberships', filter: `user_id=eq.${state.authUser.id}` },
        async (payload) => {
          log('realtime: membership updated', payload.new);
          const row = payload.new;
          await fetchMyGroups();
          renderGroupSwitcher();
          // Если изменили роль в текущей группе — обновляем UI
          if (state.currentGroup && row.group_id === state.currentGroup.group_id) {
            const updated = state.myGroups.find(g => g.group_id === row.group_id);
            if (updated && updated.role !== state.currentGroup.role) {
              state.currentGroup = updated;
              toast(`Ваша роль изменена: ${ROLE_LABELS[updated.role] || updated.role}`, 'info');
              renderShell();
              const def = getDefaultPageForRole(updated.role);
              if (def) loadPage(def);
            } else if (updated) {
              state.currentGroup = updated;
            }
...
function unsubscribeFromMemberships() {
  if (membershipsChannel) {
    try { sb.removeChannel(membershipsChannel); } catch {}
    membershipsChannel = null;
  }
}
